...
# Add project root to path to access backend
...
if "storage" not in st.session_state:
    try:
        # Use project data directory for storage
        data_dir = project_root / "data"
        chroma_dir = data_dir / "chroma"
        chroma_dir.mkdir(parents=True, exist_ok=True)
        
        st.session_state.storage = GraphMindStorage(chroma_dir, settings.EMBEDDING_MODEL)
        ...
        logger.info("GraphMind initialized successfully")
    ...
    try:
        ...
        if graph.number_of_nodes() == 0:
            ...
            if G.number_of_nodes() > 0:
                ...
                st.info("No nodes to display after filtering.")
    ...
    if st.button("🗑️ Clear All Data", type="secondary"):
        if st.checkbox("I understand this will delete all data"):
            try:
                ...
                logger.exception("Clear data error")
